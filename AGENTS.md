...
构建时设 `VUE_APP_LOCALE`（`.env` 文件里改，或 `VUE_APP_LOCALE=zh-CN pnpm build:prod`）：

- **空**（仓库里三个 `.env` 都是空的）——访客自己决定：他选过的 → 浏览器语言 → 中文
- **`zh-CN` / `en-US`** ——全站固定成这门语言，**导航栏和登录页的切换器整个不渲染**，
  访客之前存过的选择也不再生效

内网只有一门语言、或者存量部署升级后想让所有人继续看中文，用这个。

**值拼错会让构建失败**（`zh_CN`、`zh`、`cn` 都会），不会被忽略——否则界面看起来
一切正常、只是仍然跟着浏览器走，而唯一会发现的人是以为自己已经锁上的运维。
判据是 `src/lang/` 下有没有这门语言的目录，所以**加语言时目录和 `LOCALES` 必须同时改**，
`tests/unit/lang/locales.spec.ts` 有一条断言盯着这两边。

固定模式下**不写 `localStorage`**：那是访客自己的选择，不该被部署配置覆盖。
否则哪天去掉这个变量重新构建，每个人都会被钉在他从没选过的语言上。

这是**构建期**变量，`define` 静态替换，改了要重新构建；`dist/` 拿到手再改是改不了的。

跑 e2e 时**别设它**——切换器一消失，`tests/e2e/mocked/i18n.spec.ts` 会整片变红。
