        ...
        <el-row :gutter="10" class="mb10">
          ...
          <el-col :sm="24" :md="8">
            <el-card v-if="info.disk" class="box-card">
              ...
              <div class="monitor">
                <div class="monitor-header">
                  <el-progress :color="$store.state.settings.theme" type="circle" :percentage=" `${((1-info.disk.free / info.disk.total) * 100).toFixed(2)}` " />
                </div>
                <div class="monitor-footer">
                  <Cell label="总容量" :value="info.disk.total+'G'" border />
                  <Cell label="已使用" :value="`${info.disk.total-info.disk.free}`+'G'" />
                </div>
              </div>
            </el-card>
          </el-col>
